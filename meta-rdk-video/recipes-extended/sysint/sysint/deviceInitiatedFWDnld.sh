...
updateUpgradeFlag () {
    echo "dnld13--------------------devicetype is--"$DEVICE_TYPE
    if [ "$DEVICE_TYPE" == "mediaclient" ] || [ "$DEVICE_TYPE" == "broadband" ] || [ "$DEVICE_TYPE" == "camera" ] || [ "$DEVICE_TYPE" == "hybrid" ]; then
...
imageDownloadToLocalServer () {
    ...
    echo "ckp110-3-3-----upgrade protoc ----------------"$UPGRADE_PROTO
    ...
    if [ $UPGRADE_PROTO -eq 1 ]; then
        echo "ckp110-3-4-----calling tftp download should be after part creat!!!!!!!!!!!!!!!!----------------"
        #tftpDownload
        #ret=$?
    fi
    ...
    if [ $ret -ne 0 ]; then
        ...
        elif [ "$DEVICE_TYPE" == "mediaclient" ] || [ "$DEVICE_TYPE" == "broadband" ] || [ "$DEVICE_TYPE" == "camera"  ] || [ "$DEVICE_TYPE" == "hybrid" ]; then
...
getServURL()
{
    ...
    if [ -f $PERSISTENT_PATH/swupdate.conf ] && [ $buildType != "prod" ] ; then
        ...
        case $buildType in
        "prod" )
            CLOUD_URL=$CLOUDURL;;   # Pdn server URL
        "vbn" )
            CLOUD_URL=$CLOUDURL;;   # VBN server URL
        "qa" )
            # QA server URL
            CLOUD_URL=$CLOUDURL;;
        * )
...
checkForValidPCIUpgrade () {
    ...
    echo "109-2------------------------"$wrongConfigCheck
    rebootFlag=1
    echo "109-3------------------------"$cloudImmediateRebootFlag
    if [ "$cloudImmediateRebootFlag" = "false" ]; then
        rebootFlag=0
    fi

    echo "109-4------------------------"$triggerType
    if [ $triggerType -eq 1 ] ; then
...
checkForUpgrades () {
    ret=0
    echo "ckp106----------------"$pci_upgrade_status
    # PCI Upgrades
    pci_upgrade=0
...
createJsonString () {
   ...
   elif [ "$DEVICE_TYPE" == "camera" ]; then
      if [ "`ifconfig eth0 | grep "inet addr" | cut -c11-19`" == "inet addr" ] ; then
         estbMac=`ifconfig eth0 | grep HWaddr | cut -c39-55`
      else
         estbMac=`ifconfig wlan0 | grep HWaddr | cut -c39-55`
      fi
   fi
...
sendXCONFTLSRequest () {
    ...
    do
        ...
        echo "ckp77--------curl res-----------------"$curl_result
...
sendJsonRequestToCloud()
{
    ...
    echo "CKP:Inside sendJsonRequestToCloud-------------cloud url is"$CLOUD_URL
    sendXCONFRequest 
    ret=$?    
    ...
    if [ $ret -ne 0 ] ; then
        ...
        if [ $resp -ne 0 ] ; then
          echo "`Timestamp` processing response failed..while flashing the image..exiting"
          exit 1
        fi
    fi
    return $ret   
...
while [ $ret -ne 0 ]
do
    echo "CKP:send json req to cloud------------"
    sendJsonRequestToCloud $FILENAME
    echo "dnldz11-----------------"$FILENAME
    ret=$?
